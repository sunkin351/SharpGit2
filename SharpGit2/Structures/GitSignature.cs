...
namespace SharpGit2
{
    public unsafe readonly struct GitSignature
    {
        ...
        public override string ToString()
        {
            ...
            int offset = When.TotalOffsetMinutes;
            if (offset != 0)
            {
                char sign = offset < 0 ? '-' : '+';
                var (hours, minutes) = Math.DivRem(Math.Abs(offset), 60);

                if (buffer.TryWrite($"{Name} <{Email}> {When.ToUnixTimeSeconds()} {sign}{hours:00}{minutes:00}", out int written))
                {
            ...
            {
                if (buffer.TryWrite($"{Name} <{Email}> {When.ToUnixTimeSeconds()}", out int written))
                {
                    return buffer[..written].ToString();
                }
            }
            ...
            builder.Append(Name)
                ...
                .Append(Email)
                .Append("> ")
                .Append((ulong)When.ToUnixTimeSeconds());
            ...
            if (offset != 0)
            {
                ...
                builder.Append($"{hours:00}{minutes:00}");
            }
